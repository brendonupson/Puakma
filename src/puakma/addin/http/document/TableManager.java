...
public class TableManager implements ErrorDetect
{
	...
	private boolean saveRowInternal(String sWhereClause, boolean bUpdate, String sGeneratedKeyFieldName)
	{
		...
		//sGeneratedKeyFieldName=="" means DO NOT get generated keys. NULL means no clue what the key is, try to find it
		boolean bIsGettingGeneratedKeys = sGeneratedKeyFieldName==null || sGeneratedKeyFieldName.length()>0;
		...
		{
			cx = getConnection();      
			if(bUpdate || !bIsGettingGeneratedKeys)
			...
			if(!bUpdate && bIsGettingGeneratedKeys) //if an insert
			{     
				try{
